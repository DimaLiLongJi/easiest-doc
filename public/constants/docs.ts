    ...
    //   },
      {
        ...
        p: [
          ...
          `InDiv 支持双向数据绑定，这是一种对模板中的各个部件与组件中的各个部件进行协调的机制。 往模板HTML字符串中添加绑定标记可以告诉 InDiv 该如何连接它们。因为 InDiv 使用单向数据流，所以仅仅支持使用 this.state内的值作为绑定数据， 实例的方法作为事件方法。如果要在组件内使用 props ，请在 ReceiveProps 声明周期内用 props 对 state 赋值。`,
        ],
        code: `
  @Component({
    template: ('
