    ...
    info: [
      {
        ...
        p: [
          ...
          '2. template: string; 视图模板，用来生产渲染的视图',
          '3. state?: any; 提供渲染的默认state，可以不写，但在生命周期 constructor 或 nvOnInit 里声明出来',
        ],
        code: `
  @Component({
    ...
    //   },
      {
        ...
        p: [
          ...
          `InDiv 支持双向数据绑定，这是一种对模板中的各个部件与组件中的各个部件进行协调的机制。 往模板HTML字符串中添加绑定 nv- 开头的标记可以告诉 InDiv 该如何渲染它们。因为 InDiv 使用单向数据流，所以仅仅支持使用 this.state 内的值作为绑定数据， 实例的方法作为事件方法。如果要在组件内使用 props ，请在 nvReceiveProps 或 nvOnInit 生命周期内用 props 对 state 赋值。`,
        ],
        code: `
  @Component({
    selector: 'container-component',
    ...
    public nvReceiveProps(nextProps: any): void {
      ...
      },
      {
        ...
        p: [
          ...
          '例如在下面例子，在 hero-component 内可以用 state.a repeat的value persion 并且可以直接在实例方法中触发 handelClick 回调',
        ],
        code: `
  @Component({
